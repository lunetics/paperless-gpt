...
| `LLM_TEMPERATURE`                   | (Ollama metadata only) Sampling temperature for title, tag, and other metadata generation. Defaults to `0` when unset or empty. A non-negative finite number overrides it; invalid, negative, `NaN`, and `Inf` values are ignored with a warning. It does not apply to other LLM providers or Vision OCR; use `VISION_LLM_TEMPERATURE` for supported vision providers. | No       | 0                          |
